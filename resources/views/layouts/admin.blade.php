...
<head>
    ...
    <link id="darkModeStylesheet" href="{{ asset('css/dark-mode.css') }}" rel="stylesheet" disabled>
    <link id="colorlessModeStylesheet" href="{{ asset('css/colorless-mode.css') }}" rel="stylesheet" disabled>
    <link id="darkestModeStylesheet" href="{{ asset('css/darkest-mode.css') }}" rel="stylesheet" disabled>
    <link href="https://unpkg.com/gridjs/dist/theme/mermaid.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bs-stepper/dist/css/bs-stepper.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-switch/3.3.4/css/bootstrap3/bootstrap-switch.min.css">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" rel="stylesheet">
    <link rel="stylesheet" href="{{ asset('css/tutar-input.css') }}">
    <link rel="stylesheet" href="{{ asset('css/components/table-container.css') }}">
...
<ul class="navbar-nav ml-auto">
    ...
    <li class="nav-item dropdown">
        <button id="themeToggle" class="btn btn-dark btn-sm dropdown-toggle" data-toggle="dropdown" style="margin-right: 15px;">
            <i class="fas fa-adjust"></i> Tema Seç
        </button>
        <div class="dropdown-menu dropdown-menu-right">
            <a href="#" id="darkMode" class="dropdown-item"><i class="fas fa-moon"></i> Koyu Tema</a>
            <a href="#" id="darkestMode" class="dropdown-item"><i class="fas fa-adjust"></i> Tam Koyu Tema</a>
            <a href="#" id="lightMode" class="dropdown-item"><i class="fas fa-sun"></i> Normal Tema</a>
            <a href="#" id="colorlessMode" class="dropdown-item"><i class="fas fa-circle-notch"></i> Renksiz Tema</a>
        </div>
    </li>
    ...
    @endif
        ...
        <div id="loading-screen" class="d-flex justify-content-center align-items-center" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(255, 255, 255, 0.9); z-index: 1050; visibility: hidden; opacity: 0; transition: opacity 0.3s ease, visibility 0.3s ease;">
...
<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css" rel="stylesheet">
<script src="{{ asset('js/tutar-input.js') }}" defer></script>
<script src="{{ asset('js/auto-set-date.js') }}" defer></script>
<script src="{{ asset('js/utils/scroll-to-bottom.js') }}" defer></script>
...
document.addEventListener('DOMContentLoaded', function () {
    const sidebarToggleButton = document.querySelector('[data-widget="pushmenu"]');  // Sidebar toggle button
    const currentState = localStorage.getItem('sidebarState') || 'expanded';  // Varsayılan olarak 'expanded' (açık)

    // Sayfa yüklendiğinde sidebar'ı duruma göre aç veya daralt
    ...
    // Sidebar'ı daraltıp genişletmeye tıklama olayını kaydet
    sidebarToggleButton.addEventListener('click', function () {
        if (document.body.classList.contains('sidebar-collapse')) {
            // Menü daraltılmış, genişlet
            localStorage.setItem('sidebarState', 'expanded');  // Durumu genişletilmiş olarak kaydet
        } else {
            // Menü genişletilmiş, daralt
            localStorage.setItem('sidebarState', 'collapsed');  // Durumu daraltılmış olarak kaydet
        }
    });
});
    ...
    const colorlessModeStylesheet = document.getElementById('colorlessModeStylesheet');
    const darkestModeStylesheet = document.getElementById('darkestModeStylesheet');
    const themeToggle = document.getElementById('themeToggle');

    // Tema seçenekleri
    const darkMode = document.getElementById('darkMode');
    const lightMode = document.getElementById('lightMode');
    const colorlessMode = document.getElementById('colorlessMode');
    const darkestMode = document.getElementById('darkestMode');

    // Tema değiştirici işlev
    function setTheme(mode) {
        if (mode === 'dark') {
            darkModeStylesheet.disabled = false;
            colorlessModeStylesheet.disabled = true;
            darkestModeStylesheet.disabled = true;
            localStorage.setItem('theme', 'dark');
            themeToggle.innerHTML = '<i class="fas fa-moon"></i> Koyu Tema';
        } else if (mode === 'colorless') {
            darkModeStylesheet.disabled = true;
            colorlessModeStylesheet.disabled = false;
            darkestModeStylesheet.disabled = true;
            localStorage.setItem('theme', 'colorless');
            themeToggle.innerHTML = '<i class="fas fa-circle-notch"></i> Renksiz Tema';
        } else if (mode === 'darkest') {
            darkModeStylesheet.disabled = true;
            colorlessModeStylesheet.disabled = true;
            darkestModeStylesheet.disabled = false;
            localStorage.setItem('theme', 'darkest');
            themeToggle.innerHTML = '<i class="fas fa-adjust"></i> Tam Koyu Tema';
        } else {
            darkModeStylesheet.disabled = true;
            colorlessModeStylesheet.disabled = true;
            darkestModeStylesheet.disabled = true;
            localStorage.setItem('theme', 'light');
            themeToggle.innerHTML = '<i class="fas fa-sun"></i> Normal Tema';
        }
    }

    // Tema seçimi butonlarına tıklama olayları
    ...
    colorlessMode.addEventListener('click', function () {
        setTheme('colorless');
    });

    darkestMode.addEventListener('click', function () {
        setTheme('darkest');
    });
